...
search_dir = r"c:\Users\Usuario Legal\Documents\carpetas de escritorio\Proyectos Software de sunass para continuar y programar y probar localmente\sunass-portal\sunass-plus---sunassplus\web-pages"

files = glob.glob(os.path.join(search_dir, "**", "*.html"), recursive=True)
...
for filepath in files:
    ...
    # Fix downloads text wrap (we can use regex because id changes)
    content = re.sub(
        r'<span class="text-sm text-slate-500 flex items-center">(<svg[^>]*>.*?</svg>)\s*<span id="downloads-',
        r'<span class="text-sm text-slate-500 flex items-center whitespace-nowrap">\1 <span id="downloads-',
        content
    )
    content = re.sub(
        r'<span class="text-sm text-slate-500 flex items-center">(<svg[^>]*>.*?</svg>)\s*<span id="duration-',
        r'<span class="text-sm text-slate-500 flex items-center whitespace-nowrap">\1 <span id="duration-',
        content
    )
